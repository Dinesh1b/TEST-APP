...
if (window.__QA_RUN_LOADED__) {
  ...
  const MODULES = [
    { name: 'Q_Setting',       icon: '⚙️', cbId: 'cb-qs'  },
    { name: 'Stock Audit A1',  icon: '📦', cbId: 'cb-sa1' },
    { name: 'Stock Audit A2',  icon: '👥', cbId: 'cb-sa2' },
    { name: 'Summary Table',   icon: '📊', cbId: 'cb-sum' },
    { name: 'Recently Audited',icon: '🕐', cbId: 'cb-rec' },
    { name: 'Audit Plan',      icon: '📋', cbId: 'cb-ap'  },
    { name: 'Create Group',    icon: '🗃️', cbId: 'cb-cg'  },
  ];
  ...
  function _runSimulate() {
    const active = MODULES.filter((m, i) => {
      const ge = window.grpEnabled || [];
      const me = window.modEnabled || [];
      if (i < 5)   return ge[0] && me[i];
      if (i === 5) return ge[1];
      if (i === 6) return ge[2] && me[6];
      return false;
    });
    if (!active.length) {
