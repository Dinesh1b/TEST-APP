...
def A_SA2(page, Config):
    ...
    def fill_item_details_bound(
        ...
        if CATEGORY2:
            ...
            page.locator(f"div.p-dropdown-panel li:has-text('{CATEGORY2}')").click()

        dialog.locator("input[formcontrolname='itemCode']").fill(ITEM_CODE2)
        page.wait_for_timeout(500)

        if ITEM_NAME2:
            dialog.locator("input[formcontrolname='itemName']").fill(ITEM_NAME2)
            page.wait_for_timeout(500)

        if COST_PRICE2:
            dialog.locator("input[formcontrolname='costPrice']").fill(COST_PRICE2)
            page.wait_for_timeout(500)

        if SELL_PRICE2:
            dialog.locator("input[formcontrolname='sellPrice']").fill(SELL_PRICE2)
            page.wait_for_timeout(500)

        if LOCATION2:
            select_location_tree(page, LOCATION2)

        if BARCODE2:
            dialog.locator("input[formcontrolname='eanQr']").fill(BARCODE2)
            page.wait_for_timeout(500)

        if UOM2:
            dialog.locator("input[formcontrolname='uom']").fill(UOM2)
            page.wait_for_timeout(500)
    ...
    def fill_item_details_unbound(
        ...
        page.fill('input[formcontrolname="auditedQty"]', str(audited_qty2))
        page.fill('input[formcontrolname="damagedQty"]', str(damaged_qty2))
    ...
    def load_items_from_excel() -> List[Tuple[str, str, float, float]]:
        ...
        sheet_name  = Config.asa_sheet
        location_col = Config.asa_loc_col
        code_col     = Config.asa_code_col
        audited_col  = Config.asa_aud_col
        damaged_col  = Config.asa_dam_col
    ...
    def navigate_to_ongoing_audits(page, Config):
        ...
        if Config.run_new_add_item2:
            new_add_item(page)
        else:
            print("❌ New add item skipped")

        print(f"Processed {Config.ITEM_CODE2} ({Config.audited_qty2}/{Config.damaged_qty2}/{Config.LOCATION2})")
        detect_feedback(page)
        page.wait_for_timeout(500)

        # ============================================
        # Load items from Excel (Auditor 2 config)
        # ============================================
        print("\n" + "-" * 70)
        print("🔷 ADDING ITEMS FROM EXCEL (Inline Form)")
        print(f"   Sheet   : {Config.EXCEL_SHEET_auditor_2}")
        print(f"   Loc col : {Config.EXCEL_LOCATION_COL2}")
        print(f"   Code col: {Config.EXCEL_CODE_COL2}")
        print(f"   Aud col : {Config.EXCEL_AUDITED_COL2}")
        print(f"   Dam col : {Config.EXCEL_DAMAGED_COL2}")
        print("-" * 70)
