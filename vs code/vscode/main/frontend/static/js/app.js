...
if (window.__QA_APP_LOADED__) {
  ...
  /* ─────────────────────────────────────────────
     DYNAMIC REGISTRY (Fetched from /api/modules)
  ───────────────────────────────────────────── */
  window.QA_MODULES = [];
  window.QA_GROUPS = [];
  ...
  async function fetchModules() {
    try {
      ...
      window.QA_MODULES = data.modules || [];
      window.QA_GROUPS = data.groups || [];

      // Populate lookup maps
      window.QA_MODULES.forEach(m => {
        MOD_NAMES[m.idx] = m.display;
        MOD_CB[m.idx] = m.cb;
        MOD_TGL[m.idx] = m.tgl;
      });

      console.info('[QA] Registry loaded:', window.QA_MODULES.length, 'modules');
    } catch (err) {
      console.error('[QA] Failed to fetch modules:', err);
    }
  }
  ...
  window.navSub = function navSub(moduleIdx) {
    const mod = window.QA_MODULES.find(m => m.idx === moduleIdx);
    const grp = mod ? mod.group : 0;
  ...
  function syncModCount() {
    const grpEnabled = window.grpEnabled || [];
    const modEnabled = window.modEnabled || [];

    const tot = window.QA_MODULES.reduce((acc, m) => {
      return acc + (grpEnabled[m.group] && modEnabled[m.idx] ? 1 : 0);
    }, 0);
