...
const getConfig = () => {
  ...
  (window.QA_MODULES || []).forEach(m => {
    // 1. Map index to internal name (flag)
    IDX_TO_NAME[m.idx] = m.flag;

    // 2. Set dynamic boolean run flag
    const isModuleEnabled = !!(grp[m.group] && mod[m.idx]);
    runFlags[m.flag] = isModuleEnabled;

    // Special cases: Audit Plan handles timezone/adhoc sub-flags
    if (m.idx === 6) {
      runFlags[m.flag] = isModuleEnabled && caOn;
      runFlags['run_timezone'] = isModuleEnabled && tzOn;
      runFlags['run_create_Ad_hoc_Audit'] = isModuleEnabled && ahOn;
    }
  });
  ...
  const _order = (typeof window.getQueueOrder === 'function')
    ...
    : (() => {
      const out = [];
      (window.QA_MODULES || []).forEach(m => {
        if (grp[m.group] && mod[m.idx]) out.push(m.idx);
      });
      return out;
    })();
  ...
  const run_new_add_item2 = !!(grp[1] && mod[9] && bv('cb-nai2'));
