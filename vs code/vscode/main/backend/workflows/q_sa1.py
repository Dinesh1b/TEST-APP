...
import pandas as pd
from playwright.sync_api import TimeoutError as PlaywrightTimeoutError, sync_playwright, Page
...
def Q_SA1(page, Config):
    ...
    def load_items_from_excel(
                path: str,
                sheet_name= Config.EXCEL_SHEET_auditor_2,
                ...
                df = pd.read_excel(path, sheet_name= Config.EXCEL_SHEET_auditor_2)
    ...
    items_to_add = load_items_from_excel(
                        Config.EXCEL_PATH,
                        sheet_name=Config.EXCEL_SHEET_auditor_2,
                        code_col=Config.EXCEL_CODE_COL2,
                        audited_col=Config.EXCEL_AUDITED_COL2,
                        damaged_col=Config.EXCEL_DAMAGED_COL2,
                    )
    # --------------------------------------------------------------
    ...
    #
            ...
            print("✅ 'Add Item' modal opened")
            
            # fill item details
            fill_item_details_bound(
            page,
            item_code=Config.code2,
            location=Config.location2,
            audited_qty=Config.aud_qty2,
            damaged_qty=Config.dam_qty2
        )
                    
            print(f"Processed {Config.code2} ({Config.aud_qty2}/{Config.dam_qty2}/{Config.location2})")
