...
Fields to validate = columns present in 'auditSummary' Excel sheet.
  ...
  Add a column    â†’ that field gets validated
  Remove a column â†’ that field is skipped
  No code changes needed.

FILTER MERGE LOGIC:
  Negative Variance badge  â†’ shows rows from: Negative Variance + Line Items Not Found
  Positive Variance badge  â†’ shows rows from: Positive Variance + Excess Line Item

BIZ RULES:
  Matched Line Item  â†’ Stock Quantity == Audited Qty
  Location Conflict  â†’ Stock Location != Audited Location
  Damaged Report     â†’ Damaged Qty > 0
"""
...
EXCEL_PATH    = r"C:\Users\dines\Music\main_qexcel.xlsx"
EXCEL_SHEET   = "auditSummary"
FILTER_COLUMN = "filter"
Branch        = "nm"
audit_name    = "Audit_TEST_162"

# â”€â”€ filter enums â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ secondary filter columns (O, P in the Excel sheet) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
SECONDARY_FILTER_COLUMNS: List[str] = [
    "Damaged Filter",
    "Location Filter",
]

# â”€â”€ filter merge map â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ full UI column â†’ td index map â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def bar(ch="â”€",w=110,col=C): print(c(ch*w,col))
def dbl(): print(c("â•"*110,B))
def section(t): dbl(); print(c(f"  â–¶  {t}",B)); dbl()

# â”€â”€ normaliser â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
class N:
    _F = re.compile(r'^-?\d+\.\d+$')
    @classmethod
    def norm(cls, v) -> str:
...
# ======================================================
# EXCEL LOADER  â† CHANGED
# ======================================================
def load_excel(path, sheet, fcol):
    df = pd.read_excel(path, sheet_name=sheet)

    # normalised name â†’ standard UI name
    ...
    sec_col_map: Dict[str, str] = {}   # excel_col_name â†’ secondary filter value
    for col in df.columns:
    ...
    for _, row in df.iterrows():
        # â”€â”€ primary filter (column Q) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # â”€â”€ secondary filters (columns O and P) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def print_load_summary(grouped, active_fields):
    ...
    for fn, rows in grouped.items():
        print(f"  {c('âœ“',G)} Excel  {fn:<30}  {len(rows):>3} rows")
    print()
    bar("â”€", 60, DIM)
...
# â”€â”€ UI row extraction â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ badge count extraction â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ carousel navigation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    while True:
        ...
        for i in range(count):
            ...
            if _badge_label(badge) == filter_name:
                ...
                print(f"  {c('â–¶ Badge clicked',G)}  {filter_name!r}  "
...
# â”€â”€ scroll + collect â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def collect_all(page, cfg) -> List[Dict]:
    try:
        ...
        print(f"  {c('âš  No table body visible â€” 0 rows for this filter',Y)}")
        return []
...
# â”€â”€ business rules â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
class BizRules:
    @staticmethod
...
# â”€â”€ failure record â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
@dataclass
class Fail:
...
class Engine:
    ...
    def compare(self, actual, expected, filter_name):
        ...
        for code, exp_row in emap.items():
            if code not in amap:
                fail_line(code,"â€”","row present","MISSING","MISSING")
                self._addf(filter_name,code,"â€”","row present","MISSING","MISSING")
                continue
        ...
        if not is_all:
            for code in amap:
                if code not in emap:
                    fail_line(code,"â€”","not in Excel","PRESENT IN UI","EXTRA")
                    self._addf(filter_name,code,"â€”","not in Excel","PRESENT IN UI","EXTRA")
        bar()
    ...
    def summary(self) -> bool:
        ...
        print(c(f"\n  âœ…  PASSED ITEMS  ({tp})",G)); bar("â”€",70,G)
        if self.passes:
            prev=None
        ...
        print(c(f"\n  âŒ  FAILED ITEMS  ({tf})",R)); bar("â”€",110,R)
        if self.failures:
            ...
            sep=f"  {'â”€'*28}  {'â”€'*6}  {'â”€'*7}  {'â”€'*5}  {'â”€'*4}  {'â”€'*6}"
            print(sep); grand=0
        ...
        if not self.failures:
            print(f"  {c('âœ…  ALL VALIDATIONS PASSED',G)}  ({tp} items checked)")
        else:
            print(f"  {c('âŒ  VALIDATION FAILED',R)}     PASS={c(str(tp),G)}    FAIL={c(str(tf),R)}")
        dbl()
...
def navigate_to_audit(page):
    page.wait_for_selector("a[href='/home/audit']", state="visible")
    page.click("a[href='/home/audit']")
    page.wait_for_load_state("networkidle")
    select_branch(page, Branch)
    row = page.locator(f"table tbody tr:has(td span:has-text('{audit_name}'))").first
...
def main():
    ...
    grouped, active_fields = load_excel(EXCEL_PATH, EXCEL_SHEET, FILTER_COLUMN)
    print_load_summary(grouped, active_fields)

    with sync_playwright() as p:
        browser, page = login(p, browser_name="chrome", environment="production")
        navigate_to_audit(page)
        engine = Engine(active_fields=active_fields)

        for filter_name in UI_FILTERS:
            section(f"FILTER  â–¶  {filter_name}")

            badge_count = click_badge(page, cfg, filter_name)

            if badge_count == 0:
                print(f"  {c('â­  Skipping â€” badge count is 0',DIM)}\n")
                continue
            ...
            if len(merge_keys) > 1:
                ...
                print(f"   {c('â† merged:',DIM)} {merged_info}", end="")
            print("\n")
